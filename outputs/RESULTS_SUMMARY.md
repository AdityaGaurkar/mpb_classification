...
|---|---|---|---|---|---|
| **Custom CNN (FEv4)** | 16.87 | 345 | 67.5 | **4.6** | 0.5 |
| SqueezeNet 1.1 | 0.72 | 695 | **2.9** | 7.7 | 1.2 |
| MobileNetV3-Small | 1.52 | **144** | 6.1 | 45.0 | 5.5 |
| EfficientNet-B0 | 4.01 | 1,004 | 16.1 | 155.9 | 8.6 |

*FLOPs are validated against PyTorch's official `FlopCounterMode` (`revision/deploy_metrics.py`
raises if the manual count differs). Grouped/depthwise convolutions are accounted for by
scaling MACs with `out_channels/groups`; this matters for MobileNetV3-Small and
EfficientNet-B0, whose MBConv blocks use depthwise convs.*

**Reading (honest):**
- **Latency: FEv4 is the clear CPU winner.** At ~4 ms CPU inference it is **~1.9× faster than
  SqueezeNet, ~11× faster than MobileNetV3-Small, and ~34× faster than EfficientNet-B0** on
  CPU, while remaining very fast on GPU. For a server-hosted web/Streamlit deployment this
  means near-instant per-request prediction — the reason the custom CNN deploys cleanly.
- **Compute: FEv4 is efficient but not the FLOPs leader.** With 345 MFLOPs it is cheaper
  than SqueezeNet (695 M) and ~3× cheaper than EfficientNet-B0 (1,004 M), but **more
  expensive than MobileNetV3-Small (144 M)**. So FEv4 is not the theoretically cheapest in
  FLOPs; its advantage is the unusually low *latency* for its compute (simple, non-depthwise
  convolutions run efficiently on general-purpose CPUs).
- **Size / parameters / on-device: FEv4 loses.** At 67.5 MB (fp32) / 16.9 M params it is the
  **largest** home — about 23× SqueezeNet and 11× MobileNetV3 in file size. It is *not* a
  lightweight or edge/on-device model in the size sense; SqueezeNet (2.9 MB) and MobileNetV3
  (6.1 MB) are, and would be preferred if the target is browsers or mobile edge devices.

**Net deployment conclusion.** The accurate characterization of the custom CNN is: a
**low-latency, from-scratch** model that is well suited to **cloud / web** telemedicine
(fast server inference, no pretrained-weight dependency), and offers moderate FLOPs —
cheaper than EfficientNet/SqueezeNet but above MobileNetV3. It is not a
