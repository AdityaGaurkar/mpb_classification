...
*FLOPs above account for grouped convolutions (MACs of a grouped conv scale with
`out_channels/groups`); MobileNetV3-Small's depthwise convolutions are therefore counted
correctly.* Generated by `revision/deploy_metrics.py` (→ `outputs/deployment_metrics.csv`).
